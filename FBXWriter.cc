...
bool FBXWriter::doit(std::string outputFilename, bool usePrototypes)
{

  m_UsePrototypes = usePrototypes;

  G4PhysicalVolumeStore* pvStore = G4PhysicalVolumeStore::GetInstance();
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
  G4SolidStore* solidStore = G4SolidStore::GetInstance();

std::cout << __func__ << " pvStore size " << pvStore->size() << std::endl;
  // Check that the geometry has been created already
  if (pvStore->size() == 0) { return false; }

  // Find the world (top) volume
  G4VPhysicalVolume* topVol = nullptr;
  for (G4VPhysicalVolume* physVol : *pvStore) {
    if (physVol->GetMotherLogical()) { continue; } // physVol is not the world volume
    topVol = physVol;
    break;
  }
  if (topVol == nullptr) { return false; }
  ...
  // Tell caller that we succeeded
  return true;

}

void FBXWriter::assignName(std::vector<std::string>* names, unsigned int index, const G4String& originalName, int select)
{
  G4PhysicalVolumeStore* pvStore = G4PhysicalVolumeStore::GetInstance();
  G4LogicalVolumeStore* lvStore = G4LogicalVolumeStore::GetInstance();
  G4SolidStore* solidStore = G4SolidStore::GetInstance();
...
void FBXWriter::writeGeometryNode(G4VSolid* solid, const std::string& solidName, unsigned long long solidID)
{
std::cout << __func__ << "\t" << solid->GetEntityType() << " \t" << solidName  << std::endl;
  if ((solid->GetEntityType() == "G4IntersectionSolid") ||
    ...
    if ((solid->GetEntityType() == "G4TwistedTubs")) {
        HepPolyhedron::SetNumberOfRotationSteps(12);// 8 12
    }
    else {
        HepPolyhedron::SetNumberOfRotationSteps(120);//80 120);
    }
    std::cout << "NumberOfRotationSteps " << HepPolyhedron::GetNumberOfRotationSteps() << std::endl;
...
void FBXWriter::writeLVModelNode(G4LogicalVolume* logVol, const std::string& lvName, unsigned long long lvID)
{
...
void FBXWriter::addModels(G4VPhysicalVolume* physVol, int replica)
{
  // Descend to the leaves of the tree
  G4LogicalVolume* logVol = physVol->GetLogicalVolume();
  for (unsigned int daughter = 0; daughter < logVol->GetNoDaughters(); ++daughter) {
      ...
      switch (axis) {
        ...
        case kRho:
          if (solid->GetEntityType() == "G4Tubs") {
            ...
            std::cerr << "Built-in volumes replicated along radius for " << solid->GetEntityType() << " (solid " << solid->GetName() << ") are not visualisable." << std::endl;
          }
          break;
        case kPhi:
...
void FBXWriter::countEntities(G4VPhysicalVolume* physVol)
{
  // Descend to the leaves of the tree
  G4LogicalVolume* logVol = physVol->GetLogicalVolume();
  for (unsigned int daughter = 0; daughter < logVol->GetNoDaughters(); ++daughter) {
...
void FBXWriter::addConnections(G4VPhysicalVolume* physVol, int replica)
{
...
void FBXWriter::writePreamble(int modelCount, int materialCount, int geometryCount)
{
  ...
  m_File << "; FBX 7.3.0 project file" << std::endl <<
        ...
        "\tCreationTime: \"" << std::asctime(now) << ":000\"" << std::endl << 
...
void FBXWriter::writePolyhedron(G4VSolid* solid, G4Polyhedron* polyhedron, const std::string& name,
...
void FBXWriter::writePVModelNode(G4VPhysicalVolume* physVol, const std::string& pvName, unsigned long long pvID)
{
...
void FBXWriter::writeSolidToLV(const std::string& lvName, const std::string& solidName, bool visible,
...
void FBXWriter::writeSolidToPV(const std::string& pvName, const std::string& solidName, bool visible,
...
void FBXWriter::writeLVToPV(const std::string& pvName, const std::string& lvName, unsigned long long pvID,
...
void FBXWriter::writePVToParentLV(const std::string& pvNameDaughter, const std::string& lvName,
...
void FBXWriter::writePVToParentPV(const std::string& pvNameDaughter, const std::string& pvName,
...
HepPolyhedron* FBXWriter::getBooleanSolidPolyhedron(G4VSolid* solid)
{
  G4VSolid* solidA = solid->GetConstituentSolid(0);
  G4VSolid* solidB = solid->GetConstituentSolid(1);
  std::cout << solid->GetName() << "  " << solid->GetEntityType() << "  " << solidA->GetName() << "  " << solidA->GetEntityType() << "  " << solidB->GetName() << "  " << solidB->GetEntityType() <<  std::endl;
  ...
  if (solid->GetEntityType() == "G4UnionSolid") {
    ...
    *result = polyhedronA->intersect(*polyhedronB);
    ...
    std::cerr << "getBooleanSolidPolyhedron(): Unrecognized boolean solid " << solid->GetName() <<
